...
#First fill in bottom sonde data for sites <1
# fld_sheet_b<-fld_sheet %>%
#   filter(site_depth<1) %>%
#   mutate(case_when(is.na(deep_temp)))
...
missing_bottom_temps<-fld_sheet %>%
  filter(site_depth>1)%>%
  filter(is.na(temp_d))%>%
  select(uniqueid,lake_id,site_id,visit,site_depth,do_mg_d,chla_sonde_d,ph_d,sp_cond_d,turb_d)
#22 missing deep sonde data overall


#Lake 207 is missing sonde data from most sites. 
# Using the NLA17 profile to fill them
nla207<-read_xlsx(paste0(userPath, "data/CIN/CH4_207_old mans lake/dataSheets/surgeDepthProfile207nla.xlsx"))
...
i=1
#loop through to fill out missing sonde data from lake 207
...
#Load Geopackage
lakes <- st_read("C:\\Users\\bdeemer/Environmental Protection Agency (EPA)/SuRGE Survey of Reservoir Greenhouse gas Emissions - Documents/lakeDsn/all_lakes_2025-01-10.gpkg", layer = "all_lakes", quiet = TRUE)
head(lakes)

points <- st_read("C:\\Users\\bdeemer/Environmental Protection Agency (EPA)/SuRGE Survey of Reservoir Greenhouse gas Emissions - Documents/lakeDsn/all_lakes_2025-01-10.gpkg", layer = "points", quiet = TRUE)
head(points)
...
for(i in 1:8){
  ...
  uniqueid<-paste(missing_bottom_temp$lake_id[i],missing_bottom_temp$site_id[i],missing_bottom_temp$visit[i])
...
## Now look for analagous sites for site 70
