...
nla07.chem <- read.table("inputData/nla2007/nla2007_chemical_conditionestimates.txt", 
                    header = TRUE, sep = ",", as.is = TRUE, na.strings = "") %>%
  select(SITE_ID, VISIT_NO, CHLA) 

nla07.site <- read.csv("inputData/nla2007/NLA2007_SampledLakeInformation_20091113.csv", 
                         header = TRUE, sep = ",", as.is = TRUE, na.strings = "") %>%
...
nla12 <- read.table("inputData/nla2012/NLA_2012_Key_Variables_Data.txt",
...
# Convert to SpatialPointsDataFrame
nla17.sf <- st_as_sf(nla17.sf, coords = c("LON_DD83", "LAT_DD83"),
                     crs = 4269) %>% # NAD83 for lat/lon
  st_transform(., 5070) # project to CONUS Albers
...
morph.sf <- sf::st_read(dsn = paste0("inputData/lMorpho/LakeMorphGdb.gdb"),
