...
dat$deep_turb <- ifelse(dat$deep_turb <= 0, 0.01, dat$deep_turb)
...
dat$shallow_turb <- ifelse(dat$shallow_turb <= 0, 0.01, dat$shallow_turb)
...
dat$deep_chla_sonde <- ifelse(dat$deep_chla_sonde <= 0, 0.01, dat$deep_chla_sonde)

dat$shallow_chla_sonde <- ifelse(dat$shallow_chla_sonde < -1, NA, dat$shallow_chla_sonde)
dat$shallow_chla_sonde <- ifelse(dat$shallow_chla_sonde <= 0, 0.01, dat$shallow_chla_sonde)
...
dat$deep_phycocyanin_sonde <- ifelse(dat$deep_phycocyanin_sonde <= 0,
                                     0.01,
                                     dat$deep_phycocyanin_sonde)

dat$shallow_phycocyanin_sonde <- ifelse(dat$shallow_phycocyanin_sonde < -1,
                                        NA,
...
#Set chlorophyll a values that are smaller than -1 to NA and small negative values to 0.1
depth_profiles_all$chla_sonde <- ifelse(depth_profiles_all$chla_sonde < -1,
                                        NA,
