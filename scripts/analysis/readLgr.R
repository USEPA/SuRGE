...
for (i in 1:length(txtFiles)) {  # loop to read and format each file #length(txtFiles) 92 is tab delimited.  must convert to comma
  ...
  gga.i <- gga.i %>% select(lab, visit,instrument, lake_id, RDate, RDateTime, CH4.d_ppm, CO2.d_ppm, H2O._ppm, GasT_C) %>% # select columns of interest
...
fix <- read.table(paste(userPath, "data/CIN/CH4_232_Sheyenne/GGA/fga_2001-12-31_f0031.txt" , sep=""),
                  ...
                  fill=TRUE) %>% # Needed to deal with empty cells in last column
  # assign data to particular field crew
  mutate(lab="CIN",visit="1",lake_id = "232b",instrument="gga")#label this b for now to distinguish from first gga file from same lake with broken clock
...
fix <- fix %>% select(lab, visit, lake_id, instrument,RDate, RDateTime, CH4.d_ppm, CO2.d_ppm, H2O._ppm, GasT_C) %>% # select columns of interest
  rename(CH4._ppm = CH4.d_ppm, CO2._ppm = CO2.d_ppm)
