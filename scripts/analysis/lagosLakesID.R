...
surge_sites <- read_xlsx(paste0(userPath, "surgeDsn/SuRGE_design_20191206_eval_status.xlsx"), na = "NA") %>%
  ...
  select(lake_id, nla17_site_id, nhd_plus_waterbody_comid, gnis_name, lat_dd83, lon_dd83)
...
lagos_links <- left_join(surge_sites, llc, by = join_by(nhd_plus_waterbody_comid == nhdplusv2_comid)) %>%
...
#                                             TRUE ~ nhd_plus_waterbody_comid))
  ...
  rename(nhd_hr_id = lake_nhdid)

lagos_links %>% filter(is.na(lagoslakeid)) %>% {dim(.)} # only two lakes w/out LAGOS record!  One is PR.

# write/read file
# write.csv(lagos_links, file=paste0(userPath,"data/siteDescriptors/lagos_links.csv"), row.names = FALSE)
# 
