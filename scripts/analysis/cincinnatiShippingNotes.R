...
chemistry_all <- 
  chemistry_all %>%
  left_join(., shipping_data, by = c("lake_id", "visit")) %>% # join to get shipping notes
  mutate(across(
    ...
    # taxonomy, etc).  Also, metals are acidifed and stored at room temperature and
    ...
    ~ case_when(
      is.na(flag) ~ ., # if no shipping issue, flags do not change
