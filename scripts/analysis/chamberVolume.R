...
fld_sheet  <- fld_sheet %>% 
  ...
  left_join(., 
            # merge on lake_id, bring in lab attribute
        lake.list %>% 
          mutate(lake_id = as.character(lake_id)) %>% # numeric in lake.list, but character in fld_sheet
          select(lake_id, lab) %>%
          distinct(lake_id, lab)) %>% # lakes with a second visit are duplicated 
  ...
  select(-lab) # only needed to calculate volume.  now we can remove
...
fld_sheet %>% filter(is.na(chm_vol_l), # missing chamber volume 
  ...
  select(lake_id, visit,  site_id, chm_vol_l, chamb_grad_a, chamb_grad_b) %>%
  print(n=Inf)
