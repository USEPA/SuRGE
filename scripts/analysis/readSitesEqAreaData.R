...
# WE REALLY DON'T NEED THE  # Field data are compiled in excel files named 'surgeDataXXX.xlsx' g
fileNames <- list.files(path = paste0(userPath, "data/"), 
                        pattern = "surgeData", # file names containing this pattern
                        recursive = TRUE) %>% # look in all subdirectories
  .[!grepl(c(".pdf|.docx|template"), ., ignore.case = TRUE)]
...
for (i in 1:length(fileNames)) {  # for each file
  ...
  data.i <- readxl::read_xlsx(path = paste0(userPath, "data/", fileNames[i]), 
                              skip = 1)
  mylist[[i]] <- data.i
