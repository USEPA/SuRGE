...
tteb.all %>% ungroup() %>%
  ...
  pivot_longer(cols = !c(lake_id, sample_type)) %>%
  left_join(y = filter(detlimits, lab == "CIN"), by = "name") %>%
  filter(name < "cr") %>% # plot al:cd only
  ggplot() +
  aes(lake_id, value) +
  geom_point(aes(color = sample_type)) +
  geom_hline(aes(yintercept=mdl, linetype = "minimum detection limit"),
...
tteb.all %>% ungroup() %>%
  ...
  pivot_longer(cols = !c(lake_id, sample_type)) %>%
  left_join(y = filter(detlimits, lab == "CIN"), by = "name") %>%
  filter(name > "cd" & name < "mn") %>% # plot cr:mg only
  ggplot() +
  aes(lake_id, value) +
  geom_point(aes(color = sample_type)) +
  geom_hline(aes(yintercept=mdl, linetype = "minimum detection limit"),
  ...
  labs(title = "Metals field blanks",
       x =NULL, y = "mg/L") +
  theme(axis.text.x = element_text(angle = 90)) + 
  facet_wrap(~name, scales = "free", ncol = 2, nrow = 3) +
  theme(legend.position = "bottom")
...
tteb.all %>% ungroup() %>%
  ...
  pivot_longer(cols = !c(lake_id, sample_type)) %>%
  left_join(y = filter(detlimits, lab == "CIN"), by = "name") %>%
  filter(name > "mg" & name < "si") %>% # plot mn:sb only
  ggplot() +
  aes(lake_id, value) +
  geom_point(aes(color = sample_type)) +
  geom_hline(aes(yintercept=mdl, linetype = "minimum detection limit"),
...
tteb.all %>% ungroup() %>%
  ...
  pivot_longer(cols = !c(lake_id, sample_type)) %>%
  left_join(y = filter(detlimits, lab == "CIN"), by = "name") %>%
  filter(name > "s" & name < "tteb.doc") %>% # plot si:sr only
  ggplot() +
  aes(lake_id, value) +
  geom_point(aes(color = sample_type)) +
  geom_hline(aes(yintercept=mdl, linetype = "minimum detection limit"),
...
tteb.all %>% ungroup() %>%
  ...
  pivot_longer(cols = !c(lake_id, sample_type)) %>%
  left_join(y = filter(detlimits, lab == "CIN"), by = "name") %>%
  filter(name > "sr") %>% # plot tteb:zn only
  ggplot() +
  aes(lake_id, value) +
  geom_point(aes(color = sample_type)) +
  geom_hline(aes(yintercept=mdl, linetype = "minimum detection limit"),
