...
name = c("tno2_3", "tnh4", "trp", "tp", "tn", "toc", "chla", "al", "as", "ba", "be", 
         "ca", "cd", "cr", "cu", "fe", "k", "li", "mg", "mn", "na", "ni", "pb", "p", 
         "sb", "si", "sn", "sr", "s", "v", "zn", "no2_3", "no2", "no3", "nh4", "op", 
         "tp", "tn", "toc", "doc", "br", "cl", "so4", "f")
mdl = c(6, 6, 3, 5, 25, 100, 9, 4, 4, 1, 5, 10, 0.3, 1, 1, 1, 300, 5, 5, 1, 30, 1,
        2, 5, 3, 20, 1, 1, 3, 1, 0.5, 0.01, 0.012, 0.01, 0.00171, 0.002, 0.002, 0.002, 
        0.102, 0.102, 0.041, 0.026, 0.151, 0.014)
# need to add chl-a detection limit when available
detlimits <- data.frame(name, mdl) 
...
### CIN Anions
Anion samples collected in 2020 by field crews other than ADA were analyzed at EPA Cincinnati.  All bromide blanks were below the detection limit, as where most of the unknowns.  Most chloride blanks were below the detection limit and all were several orders of magnitude below the unknowns.  Lake 233 had a high F blank, but all others were at DL.  Sulfate blanks were near the detection limit and serveral orders of magnitude below the unknowns.
...
d.anions %>% 
  ...
  select(-max_blank) %>%
  rename(anion = name, blank = min_blank, minimum = min_unknown, maximum = max_unknown) %>%
  relocate(lake_id, anion, blank, minimum, maximum) %>%
  kableExtra::kbl(booktabs = TRUE, # nice formatting
