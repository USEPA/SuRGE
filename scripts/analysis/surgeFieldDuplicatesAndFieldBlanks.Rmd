...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# set up needed for all analyte groups below
...
chemistry_all_dups <- chemistry_all %>% 
  ...
  mutate(difference = abs(diff(value)), # difference between dup and unknown
         ...
         rpd = (difference/mean)*100,
         rpd_flag = case_when(rpd > 50 ~ ">50% rpd", # used to color code plots
                              TRUE ~ "<50% rpd")) %>%
  ungroup()
...
### Nutrients
```{r echo=FALSE, message=FALSE, warning=FALSE}
# set up and calculations specific to nutrients

# vector of nutrient names.  Needed to extract nutrients from chemistry_all_dups
nutrient_name <- detlimits %>% 
  filter(analyte_group == "nutrients") %>%
  select(name) %>% 
  pull()

# pull out unknowns, field duplicates, and rpd for lakes with field dups
nutrients_rpd <- chemistry_all_dups %>% # this object included rpd calculation
  filter(name %in% nutrient_name) 

# mean relative percent difference by analyte.  Aggregate across analyte
nutrients_rpd_analyte <- nutrients_rpd %>%
  distinct() %>% # currently reported for both dups
  group_by(name) %>% # group by analyte
  summarize(rpd = mean(rpd, na.rm = TRUE))


```
Relative percent agreement among nutrient duplicates was greatest for `r nutrients_rpd_analyte %>% filter(rpd == max(rpd)) %>% select(name) %>% pull()`(`r nutrients_rpd_analyte %>% filter(rpd == max(rpd)) %>% select(rpd) %>% pull() %>% round(.,1)`%) and lowest for `r nutrients_rpd_analyte %>% filter(rpd == min(rpd)) %>% select(name) %>% pull()`(`r nutrients_rpd_analyte %>% filter(rpd == min(rpd)) %>% select(rpd) %>% pull() %>% round(.,1)`%).  Plots of measured nutrient concentrations in unknowns and field duplicates don't reveal any obvious issues.  Lake 191 nh4 and tp unknown and field duplicate differ considerably, but values for both samples are realistic.


```{r echo=FALSE, message=FALSE, warning=FALSE}
# table of mean RPD by analyte
nutrients_rpd_analyte %>%
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=11}
# figure of unknowns and corresponding field duplicate
ggplot(nutrients_rpd, aes(lake_id, value)) +
  geom_point(aes(color = rpd_flag)) + # color by rpd value
  ylab("ug N or P /L") +
  facet_wrap(~name, scales = "free") +
...
ada.anions %>% 
  ...
  summarize(min = round(min(value),2),  max = round(max(value),2)) %>% 
  pivot_wider(names_from = sample_type, values_from = c(min, max)) %>% 
  filter(is.na(min_duplicate) == FALSE) %>%
