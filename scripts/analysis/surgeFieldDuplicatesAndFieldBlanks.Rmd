...
tteb.all %>% ungroup() %>%
  ...
  pivot_longer(cols = !c(lake_id, sample_type)) %>%
  left_join(y = filter(detlimits, lab == "CIN"), by = "name") %>%
  filter(name < "fe") %>% # plot al:mg only
  ggplot() +
  aes(lake_id, value) +
  geom_point(aes(color = sample_type)) +
  geom_hline(aes(yintercept=mdl, linetype = "minimum detection limit"),
...
ggplot(chemistry_all_dups %>% 
  ...
  xlab("ug N or P / L") +
  facet_wrap(~name, scales = "free") +
  ggtitle("Concentrations of Field Duplicates and Paired Unknowns: nutrients") +
...
ggplot(chemistry_all_dups %>% 
       ...
       aes(lake_id, value)) +
  geom_point() +
  ggtitle("Concentration of Field Duplicates and Paired Unknowns: anions") +
  facet_wrap(~name, scales = "free") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
