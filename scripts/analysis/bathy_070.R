...
boundary_70 <- st_read(paste0(userPath, "lakeDsn/CIN/CH4-070/eqArea070_Merge.shp")) %>%
  # not certain if "source" is needed, following blog post
  # setting depth = 0 at boundary
  transmute(source = "boundary", depth = 0) %>%
  st_transform(st_crs(measured_depths_70)) # same CRS as depth soundings
...
measured_depths_70 <- st_read(paste0(userPath, "lakeDsn/CIN/CH4-070/bathymetry/Lake_Francis_Case_Hydro_1000_5_fall2022_NAVD88.shp")) %>%
