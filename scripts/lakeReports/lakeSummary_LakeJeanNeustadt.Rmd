...
Turbidity is higher near shoreline and shallow sampling stations. Water temperature is highest in the relatively shallow sites at the north end of the reservoir. Dissolved oxygen and pH are greatest near the dam, likely reflecting high rates of algal metabolism in the relatively warm and clear water near the dam. 
...
The deepest sampling location in `r lake_name_i` was `r chem_fld_i %>% filter(lake_id == as.numeric(lake_id_i), site_id == index_site) %>% select(site_depth) %>% distinct() %>% pull()` m deep. Thermal stratification was observed in `r lake_name_i`, and dissolved oxygen was nearly depleted at the lake bottom, indicating strong biological oxygen demand in the lake sediment and reduced oxygen replenishment likely driven by thermal stratification.  
