...
# Filter out any missing temperature data
ptsTemp <- pts %>% 
  filter(!is.na(temp_s)) 

# %>% 
#   mutate(site_id = paste("siteID:", site_id)) # add text to site_id for label

factpal.points <- leaflet::colorNumeric(palette = c('#67a9cf','#f7f7f7', '#ef8a62'), 
                                        domain = ptsTemp$temp_s)



# tibble(ptsTemp) %>%
#   select(siteID, temp_s) %>%
#   filter(!is.na(temp_s)) %>%
#   kableExtra::kbl(caption = paste0("<br>", "Water Temperature")) %>%
#   kableExtra::kable_classic(full_width = FALSE,
#                             font_size = 16, position = "float_right")


bscols(widths = c(9,3), leaflet() %>% 
  addProviderTiles(providers$Esri.WorldImagery) %>%
  #setView(st_coordinates(cntr_crds)[1], st_coordinates(cntr_crds)[2], zoom = 15) %>%
  ...
  addLegend_decreasing(position = "topright", 
                       pal = factpal.points,
                       opacity = 1,
                       values = ptsTemp$temp_s,
                       title = "Water Temperature (C)",
                       decreasing = TRUE) %>%
  addScaleBar(),
  datatable(select(st_drop_geometry(ptsTemp), siteID, temp_s), 
            options = list(dom = "t", pageLength = 20), width = "50%", 
