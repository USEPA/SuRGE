...
The Sylvan Lake survey design included 15 sampling sites.  Water chemistry samples were collected from a `r chem_fld_i %>% filter(lake_id == 235, site_id == 3) %>% select(site_depth) %>% distinct() %>% pull()`m deep site nearby the dam (Figure 2).  Click onany of the sites to see the site id, water temperature, pH, turbidity, and dissolved oxygen at the water surface.
...
A field sensor was used to measure water temperature, pH, dissolved oxygen, and turbidity near the water surface at all sampling sites.  Data are reported in figures and tables below.  Hover the curser over any point in the figures to reveal the  siteID.
...
# # dissolved oxygen
# tibble(pts_sp@data) %>% 
#   select(siteID, do_mg_s) %>%
#   filter(is.na(do_mg_s) == FALSE) %>%
#   kableExtra::kbl(caption = paste0("<br>", "Dissolved Oxygen")) %>%
#   kableExtra::kable_classic(full_width = FALSE, 
#                             font_size = 16, position = "float_right") 
...
bscols(widths = c(9,3), leaflet(turbdata) %>% 
  ...
  datatable(turbdata, width = "50%", class = "compact", 
            rownames = FALSE,# resize 
            colnames= c("NTU" = "turb_s")) #rename column
)
