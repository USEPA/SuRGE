...
# I think the datatable() function only identifies columns by number, not name.
...
bscols(widths = c(9,3), leaflet(tempdata) %>% 
  ...
  addScaleBar(),
  datatable(tempdata, 
            options = list(dom = "t", # show table only (no buttons)
                           columnDefs = list(list(visible=FALSE, 
                                                  targets = c(2))),# hide column
                           pageLength = 20), # show up to 20 sites
            width = "50%", class = "compact", rownames = FALSE,# resize 
...
# tibble(pts_sp@data) %>% 
#   select(siteID, ph_s) %>%
#   filter(is.na(ph_s) == FALSE) %>%
#   kableExtra::kbl(caption = paste0("<br>", "pH")) %>%
...
# # turbidity
# tibble(pts_sp@data) %>% 
#   select(siteID, turb_s) %>%
#   filter(is.na(turb_s) == FALSE) %>%
#   kableExtra::kbl(caption = paste0("<br>", "Turbidity")) %>%
#   kableExtra::kable_classic(full_width = FALSE, 
#                             font_size = 16, position = "float_right") 
# 
# # dissolved oxygen
