...
sample_layer_os <- "main_OverSampSitesMerc317" # e.g., "main_mainSitesMerc233"
...
The `r lake_name_i` survey design included 15 sampling sites that were sampled on `r chem_fld_i %>% filter(!is.na(sample_date)) %>% distinct(sample_date) %>% filter(sample_date == min(sample_date)) %>% pull()`.  Water chemistry samples were collected from a `r chem_fld_i %>% filter(lake_id == as.numeric(lake_id_i), site_id == index_site) %>% select(site_depth) %>% distinct() %>% pull()`m deep site (Figure 2).  Click on any of the sites to see the site id, water temperature, pH, turbidity, and dissolved oxygen at the water surface.
...
pts <- read_sf(paste0(userPath, gdb_name), 
               layer = sample_layer) %>%
  filter(!(siteID %in% c("U-06", "U-01"))) %>% # inaccessible main sites
  st_transform(4326) 

# Read oversample sites.  Some of these were used in survey.
pts.os <- read_sf(paste0(userPath, gdb_name),
                  layer = sample_layer_os) %>%
  filter(siteID %in% c("U-16", "U-18")) %>% # oversample sites used
  st_transform(4326)

pts <- rbind(pts, pts.os) # This has all sampled sites.
...
Turbidity was generally higher and water temperatures were slightly warmer on the north side of the lake. pH was consistent throughout the lake, likely reflecting similar rates of algal metabolism throughout the reservoir. Dissolved oxygen was generally high, with one outlier in the middle of the reservoir where the dissolved oxygen concentration was only `r pts %>% filter(siteID == 13) %>% select(do_mg) %>% as.data.frame() %>% pull(do_mg)` mg L^-1^.
...
The deepest sampling location in `r lake_name_i` was `r chem_fld_i %>% filter(lake_id == as.numeric(lake_id_i), site_id == index_site) %>% select(site_depth) %>% distinct() %>% pull()` m deep. In relatively shallow lakes like `r lake_name_i`, wind induced mixing of the water column is often sufficient to prevent thermal stratification. `r lake_name_i` had only very minor thermal stratification, yet dissolved oxygen declined from a concentration of `r depth_prf %>% arrange(sample.depth.m) %>% select(do.mg.l) %>% slice(1) %>% pull(do.mg.l)` mg L^-1^ near the surface to `r depth_prf %>% arrange(desc(sample.depth.m)) %>% select(do.mg.l) %>% slice(1) %>% pull(do.mg.l)` mg L^-1^ near the lake bottom.  This indicates some degree of biological oxygen demand in lake sediment.
