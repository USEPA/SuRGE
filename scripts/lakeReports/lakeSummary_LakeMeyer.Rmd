...
l <- leaflet() %>% 
  ...
  addPolygons(data = st_zm(ecoR), fillColor = ~factpal(WSA9_NAME), stroke = FALSE, fillOpacity = 0.5) %>%
...
The `r lake_name_i` survey design included 15 sampling sites that were sampled on `r chem_fld_i %>% filter(!is.na(sample_date)) %>% distinct(sample_date) %>% filter(sample_date == min(sample_date)) %>% pull()`.  Water chemistry samples were collected from a `r chem_fld_i %>% filter(lake_id == as.numeric(lake_id_i), site_id == index_site) %>% select(site_depth) %>% distinct() %>% pull()`m deep site nearby the dam (Figure 2).  Click on any of the sites to see the site id, water temperature, pH, and dissolved oxygen at the water surface.
...
pts <- read_sf(paste0(userPath, gdb_name), 
               layer = sample_layer) %>%
  st_transform(4326) %>%
  filter(siteID != "U-13") # inacessible
...
disturbance_i <- cbind(depth_prf_mean, nutrients) %>% # join nutrients and 0-2m sonde data
  ...
  mutate(concentration = case_when(parameter == "do" ~ format(concentration, digits = 0, nsmall = 0),
                                   ...
                                   parameter == "chlorophyll a" ~ format(concentration, digits = 2, nsmall = 0)))
...
disturbance_i %>%
  ...
  kableExtra::kbl(align = "lcccccc", # nice formatting
                  caption = "Chemical Condition Indicators Measured at Water Chemistry Site") %>%
  kableExtra::kable_classic() %>%
...
There were no consistent spatial patterns in water temperature, pH, or dissolved oxygen in the surface waters of the lake.
...
The deepest sampling location in `r lake_name_i` was `r chem_fld_i %>% filter(lake_id == as.numeric(lake_id_i), site_id == index_site) %>% select(site_depth) %>% distinct() %>% pull()` m deep.  In relatively shallow lakes like `r lake_name_i`, wind induced mixing of the water column is often sufficient to prevent thermal stratification.  `r lake_name_i` had minor thermal stratification and dissolved oxygen was greatest near the sediment water interface.   This is a very unusual dissolved oxygen profile, but is sometimes seen when lake managers install an aerator to maintain high dissolved oxygen in the lake.
