...
Turbidity is highest near the river inflows, but decreases toward the dam as water velocity decreases and suspended sediment drops out of the water column. Surface water temperature was also greatest near the river inflows, but varied by only `r pts %>% filter(!is.na(temp)) %>% pull(temp) %>% range() %>% diff() %>% round()` °C from location to location.  Dissolved oxygen and pH were greatest near the dam, likely reflecting high rates of algal metabolism in the clearer water near the dam.
...
The deepest sampling location in `r lake_name_i` was `r chem_fld_i %>% filter(lake_id == as.numeric(lake_id_i), site_id == index_site) %>% select(site_depth) %>% distinct() %>% pull()` m deep.  `r lake_name_i` had moderate thermal stratification, with the surface waters being about `r depth_prf %>% pull(temp.C) %>% range() %>% diff()`° C warmer than the bottom waters.  The reservoir had a metalimnetic (mid-water-column) dissolved oxygen minima (concentrations of < 1 mg L^-1^ at depths of 5-7 m).  Dissolved oxygen concentrations at the reservoir bed were higher, reaching nearly `r depth_prf %>% arrange(desc(sample.depth.m)) %>% select(do.mg.l) %>% slice(1) %>% pull() %>% round()` mg L^-1^.  The metalimnetic low oxygen indicates strong biological oxygen demand in lake sediments that was likely counteracted by the hypolimnetic oxygenation system in place on the bottom of the reservoir.  The oxygen diffusers were confirmed to have been running during both days of the survey, 17 and 18 Aug 2021 (personal communication with Chris Dembiczak).
