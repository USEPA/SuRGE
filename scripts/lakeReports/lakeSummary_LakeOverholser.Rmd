...
chem_fld_i <- chem_fld %>% filter(lake_id == as.numeric(lake_id_i)) %>%
  mutate(turb = ifelse(turb >1000, NA, turb)) # the turbidity value at site 6 is erroneous

print(select(chem_fld_i, site_id, turb))
...
Turbidity is highest near the river inflows, but decreases toward the dam as suspended sediment drops out of the water column, although turbidity is likely driven by wind mixing in this system. Water temperature is lowest near the river inflows and reflects warming of water through solar heating as it moves through the reservoir. Dissolved oxygen and pH are greatest at sampling sites where temperature is highest and turbidity is lowest, likely reflecting high rates of algal metabolism in the relatively warm and clear water.
