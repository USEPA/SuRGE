...
The `r lake_name_i` survey design included 15 sampling sites that were sampled on `r chem_fld_i %>% filter(!is.na(sample_date)) %>% distinct(sample_date) %>% filter(sample_date == min(sample_date)) %>% pull()`.  Water samples were collected for nutrient and chlorophyll analysis from a `r chem_fld_i %>% filter(lake_id == as.numeric(lake_id_i), site_id == index_site) %>% select(site_depth) %>% distinct() %>% pull()`m deep site nearby the dam (Figure 2).  Click on any of the sites to see the site id, water temperature, pH, turbidity, and dissolved oxygen at the water surface.
...
Turbidity is highest near the river inflows, but decreases toward the dam as water velocity decreases and suspended sediment drops out of the water column.  Water temperature and dissolved oxygen were also greatest near the river inflows.  
