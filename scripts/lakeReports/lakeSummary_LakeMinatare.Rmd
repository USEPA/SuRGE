...
Water temperature, dissolved oxygen, and pH showed very little spatial variation across the reservoir's surface waters.
...
The deepest sampling location in `r lake_name_i` was `r chem_fld_i %>% filter(lake_id == as.numeric(lake_id_i), site_id == index_site) %>% select(site_depth) %>% distinct() %>% pull()` m deep.  The top 5m of the water column was relatively warm, but temperature dropped rapidly below 5m, indicating strong thermal stratification.  Dissolved oxygen was also lower below 5 m.
