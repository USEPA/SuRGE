...
ggplot(chem_fld_i, aes(sample_depth_d, do_mg_d)) +
  geom_line(color = "#56B4E9", size = 2) + # primary axis data (DO)
  geom_point(color = "#56B4E9", size = 3) + # primary axis data (DO)
  geom_line(aes(y = a + temp_d*b), color = "#E69F00", size = 2) + # secondary axis data (water temp)
  geom_point(aes(y = a + temp_d*b), color = "#E69F00", size = 3) + # secondary axis data (water temp)
  scale_y_continuous(expression(paste("Dissolved Oxygen (", mg^-1, ")"), g^1), sec.axis = sec_axis(~ (. - a)/b, name = "Temperature (°C)")) + # scale and label both axes
