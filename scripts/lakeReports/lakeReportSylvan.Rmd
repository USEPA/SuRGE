...
popupData <- chem_fld_i %>% # subset popup data from chem_fld_i 
  mutate(site_id = str_c("S-", str_pad(as.character(site_id), 2, "left", "0"))) %>% # change site_id field to match ptsWgs
...
ptsWgs <- ptsWgs %>% # join popup data with ptsWgs 
  ...
  mutate(popup = paste0(siteID, "<br>", "water temp.(°C): ", temp_s, "<br>", "pH: ", ph_s, "<br>", "turbidity (NTU): ", turb_s,"<br>",  "DO (mg/L): ", do_mg_s)) # create new field with all of the popup data
