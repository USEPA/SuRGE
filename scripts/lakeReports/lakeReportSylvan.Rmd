...
#localName <- "Joe/" # R proj folder at 
...
Between 2020 and 2023 the US Environmental Protection Agency (USEPA) will survey water quality and greenhouse gas (GHG) emissions from 108 reservoirs distributed across the nine major US ecoregions (Figure 1) as part of the SuRGE project.  The objective of the research is to estimate the magnitude of GHG emissions from US reservoirs. 

All reservoirs included in this project were previously sampled by the USEPA as part of the 2017 National Lakes Assessment (NLA).  Data from this study can be downloaded [here](https://www.epa.gov/national-aquatic-resource-surveys/data-national-aquatic-resource-surveys).  `r lake_name_i` is identified with a SITE_ID value of `r nla_id_i` in the data files.

For the SuRGE project, water quality parameters (dissolved oxygen, pH, conductivity, water temperature, and turbidity) are measured near the water surface using a field sensor at a minimum of 15 locations within each reservoir.  Water samples are collected from the deepest site for analysis of nutrients and chlorophyll a.    
...
The `r lake_name_i` survey design included 15 sampling sites.  Water chemistry samples were collected from a `r chem_fld_i %>% filter(lake_id == lake_id_i, site_id == site_id_i) %>% select(site_depth) %>% distinct() %>% pull()`m deep site nearby the dam (Figure 2).  
