...
author:
- J. Beaulieu^[United States Environmental Protection Agency, Office of Research and Development, Beaulieu.Jake@epa.gov]
...
ptsWgs <- ptsWgs %>% # join popup data with ptsWgs 
  mutate(siteID = as.numeric(gsub(".*?([0-9]+).*", "\\1", siteID))) %>% 
  left_join(popupData, by = c("siteID" = "site_id")) %>% 
  mutate(popup = paste0("siteID: ", siteID, "<br>", "water temp.(°C): ", temp_s, "<br>", "pH: ", ph_s, "<br>", "turbidity (NTU): ", turb_s,"<br>",  "DO (mg/L): ", do_mg_s)) # create new field with all of the popup data
...
depth_prf_mean <- depth_prf %>% filter(sample.depth.m <= 2) %>% 
  ...
  mutate(do.mg.l = format(do.mg.l, digits = 0),
         turbidity.ntu = format(turbidity.ntu, digits = 2, nsmall = 2)) %>%
...
nutrients <- chem_fld_i %>% 
  ...
  select(tp, tn, chla) %>%
  mutate(tp = format(tp, digits = 0),
         tn = format(tn, digits = 0),
         chla = format(chla, digits = 1, nsmall = 1)) %>%
