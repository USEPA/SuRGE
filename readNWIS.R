...
saguarom<-saguaro %>%
  mutate(lake_id="288",lake_name="Saguaro")%>%
  mutate(date=as.Date(datetime,format="%Y-%m-%d"))%>%
  mutate(annual_watlev_prop=mean(sap)*100)%>%
  select(lake_id,lake_name,date,datetime,read_value_ft,annual_watlev_prop)
colnames(saguarom)<-c("lake_id","lake_name","date","dateTime","feet","annual_watlev_prop")
...
madisonm<-madison %>%
  mutate(lake_id="211",lake_name="Madison")%>%
  mutate(date=as.Date(datetime,format="%Y-%m-%d"))%>%
  mutate(annual_watlev_prop=NA)%>%
  mutate(dateTime=NA)%>%
  select(lake_id,lake_name,date,dateTime,elevation,annual_watlev_prop)
colnames(madisonm)<-c("lake_id","lake_name","date","dateTime","feet","annual_watlev_prop")
...
sanleandrom<-sanleandro %>%
  mutate(lake_id="291",lake_name="Upper San Leandro")%>%
  mutate(date=as.Date(datetime,format="%Y-%m-%d"))%>%
  mutate(annual_watlev_prop=mean(slp)*100)%>%
  mutate(dateTime=NA)%>%
  select(lake_id,lake_name,date,dateTime,average,annual_watlev_prop)
colnames(sanleandrom)<-c("lake_id","lake_name","date","dateTime","feet","annual_watlev_prop")
...
emigrantm<-emigrant %>%
  mutate(lake_id="249",lake_name="Emigrant")%>%
  mutate(date=as.Date(datetime,format="%Y-%m-%d"))%>%
  mutate(annual_watlev_prop=mean(emp)*100)%>%
  mutate(dateTime=NA)%>%
  select(lake_id,lake_name,date,dateTime,emi_fb_ft,annual_watlev_prop)
colnames(emigrantm)<-c("lake_id","lake_name","date","dateTime","feet","annual_watlev_prop")
...
arkabutlam<-arkabutla %>%
  mutate(lake_id="2",lake_name="Arkabutla")%>%
  mutate(date=as.Date(datetime,format="%Y-%m-%d"))%>%
  mutate(annual_watlev_prop=mean(arp)*100)%>%
  mutate(dateTime=NA)%>%
  select(lake_id,lake_name,date,dateTime,pool_level_ft,annual_watlev_prop)
colnames(arkabutlam)<-c("lake_id","lake_name","date","dateTime","feet","annual_watlev_prop")
...
owyheem<-owyhee %>%
  mutate(lake_id="287",lake_name="Owyhee")%>%
  mutate(date=as.Date(datetime,format="%Y-%m-%d"))%>%
  mutate(annual_watlev_prop=mean(owp)*100)%>%
  mutate(dateTime=NA)%>%
  select(lake_id,lake_name,date,dateTime,owy_fb,annual_watlev_prop)
colnames(owyheem)<-c("lake_id","lake_name","date","dateTime","feet","annual_watlev_prop")
...
philipsm<-philips %>%
  mutate(lake_id="239",lake_name="Philips")%>%
  mutate(date=as.Date(datetime,format="%Y-%m-%d"))%>%
  mutate(annual_watlev_prop=mean(php)*100)%>%
  mutate(dateTime=NA)%>%
  select(lake_id,lake_name,date,dateTime,phl_fb_feet,annual_watlev_prop)
colnames(philipsm)<-c("lake_id","lake_name","date","dateTime","feet","annual_watlev_prop")
...
boulderm<-boulder %>%
  mutate(lake_id="186",lake_name="Boulder")%>%
  mutate(date=as.Date(datetime,format="%Y-%m-%d"))%>%
  mutate(annual_watlev_prop=mean(bop)*100)%>%
  mutate(dateTime=NA)%>%
  select(lake_id,lake_name,date,dateTime,gage_ht_value,annual_watlev_prop)
colnames(boulderm)<-c("lake_id","lake_name","date","dateTime","feet","annual_watlev_prop")
...
newmelonesm<-newmelones %>%
  mutate(lake_id="297",lake_name="New Melones")%>%
  mutate(date=as.Date(datetime,format="%Y-%m-%d"))%>%
  mutate(annual_watlev_prop=mean(nmp)*100)%>%
  select(lake_id,lake_name,date,datetime,value,annual_watlev_prop)
colnames(newmelonesm)<-c("lake_id","lake_name","date","dateTime","feet","annual_watlev_prop")
...
sylvanm<-sylvan %>%
  mutate(lake_id="235",lake_name="Sylvan")%>%
  mutate(date=as.Date(dateTime,format="%Y-%m-%d"))%>%
  mutate(annual_watlev_prop=mean(syp)*100)%>%
  select(lake_id,lake_name,date,dateTime,GH_Inst,annual_watlev_prop)
colnames(sylvanm)<-c("lake_id","lake_name","date","dateTime","feet","annual_watlev_prop")
...
jacksonvillem<-jacksonville %>%
  mutate(lake_id="11",lake_name="Jacksonville")%>%
  mutate(date=as.Date(dateTime,format="%Y-%m-%d"))%>%
  mutate(annual_watlev_prop=mean(jap)*100)%>%
  select(lake_id,lake_name,date,dateTime,X_62614_Inst,annual_watlev_prop)
colnames(jacksonvillem)<-c("lake_id","lake_name","date","dateTime","feet","annual_watlev_prop")
...
hamiltonm<-hamilton %>%
  mutate(lake_id="99",lake_name="Hamilton")%>%
  mutate(date=as.Date(dateTime,format="%Y-%m-%d"))%>%
  mutate(annual_watlev_prop=mean(hap)*100)%>%
  select(lake_id,lake_name,date,dateTime,GH_Inst,annual_watlev_prop)
colnames(hamiltonm)<-c("lake_id","lake_name","date","dateTime","feet","annual_watlev_prop")
...
bakerm<-baker %>%
  mutate(lake_id="999",lake_name="Baker")%>%
  mutate(date=as.Date(dateTime,format="%Y-%m-%d"))%>%
  mutate(annual_watlev_prop=mean(bap)*100)%>%
  select(lake_id,lake_name,date,dateTime,X_62615_Inst,annual_watlev_prop)
colnames(bakerm)<-c("lake_id","lake_name","date","dateTime","feet","annual_watlev_prop")
...
whorsem<-whorse %>%
  mutate(lake_id="298",lake_name="Wild Horse")%>%
  mutate(date=as.Date(dateTime,format="%Y-%m-%d"))%>%
  mutate(annual_watlev_prop=mean(whp)*100)%>%
  select(lake_id,lake_name,date,dateTime,X_00062_Inst,annual_watlev_prop)
colnames(whorsem)<-c("lake_id","lake_name","date","dateTime","feet","annual_watlev_prop")
...
overholserm<-overholser %>%
  mutate(lake_id="167",lake_name="Overholser")%>%
  mutate(date=as.Date(dateTime,format="%Y-%m-%d"))%>%
  mutate(annual_watlev_prop=mean(ovp)*100)%>%
  select(lake_id,lake_name,date,dateTime,GH_Inst,annual_watlev_prop)
colnames(overholserm)<-c("lake_id","lake_name","date","dateTime","feet","annual_watlev_prop")
...
pinesm<-pines %>%
  mutate(lake_id="3",lake_name="Lake O' the Pines")%>%
  mutate(date=as.Date(dateTime,format="%Y-%m-%d"))%>%
  mutate(annual_watlev_prop=mean(pip)*100)%>%
  select(lake_id,lake_name,date,dateTime,X_62614_Inst,annual_watlev_prop)
colnames(pinesm)<-c("lake_id","lake_name","date","dateTime","feet","annual_watlev_prop")
...
tschidam<-tschida %>%
  mutate(lake_id="68",lake_name="Tschida")%>%
  mutate(date=as.Date(dateTime,format="%Y-%m-%d"))%>%
  mutate(annual_watlev_prop=mean(tsp)*100)%>%
  select(lake_id,lake_name,date,dateTime,GH_Inst,annual_watlev_prop)
colnames(tschidam)<-c("lake_id","lake_name","date","dateTime","feet","annual_watlev_prop")
...
francism<-francis %>%
  mutate(lake_id="70",lake_name="Francis")%>%
  mutate(date=as.Date(dateTime,format="%Y-%m-%d"))%>%
  mutate(annual_watlev_prop=NA)%>%
  select(lake_id,lake_name,date,dateTime,GH_Inst,annual_watlev_prop)
colnames(francism)<-c("lake_id","lake_name","date","dateTime","feet","annual_watlev_prop")

## Bind rows together

walev<-rbind(tschidam,pinesm,whorsem,bakerm,jacksonvillem,sylvanm,newmelonesm,
             boulderm,philipsm,owyheem,arkabutlam,sanleandrom,saguarom,
             overholserm,emigrantm,hamiltonm,francism,madisonm)
...
walev_link<-walev %>%
  group_by(lake_id)%>%
  summarise(annual_watlev_prop=annual_watlev_prop[1])
